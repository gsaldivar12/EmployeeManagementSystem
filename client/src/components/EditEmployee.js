...
const EditEmployee = () => {
  ...
  const fetchEmployee = async () => {
    try {
      ...
      setFormData({
        ...
        dateOfHire: new Date(employee.dateOfHire).toISOString().split('T')[0]
      });
    } catch (error) {
      setSubmitError(error.message || 'Error fetching employee');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/', { state: { message: 'Employee updated successfully!' } });
    } catch (error) {
      setSubmitError(error.message || 'Error updating employee');
    } finally {
      setLoading(false);
    }
  };
